...
const getRtnRedirect = async (url: RequestInfo, header?: any): Promise<string> => {
    ...
    let testWhatReq = async response => {
...
const parseCookieString = (cookieString: string) =>{
  ...
  async function refreshNotionSession() {
    ...
    // 合并新旧cookie
    const oldCookies = parseCookieString(notionInfo.cookie);
...
const tempUrl = async (url: string, name: string, cookie: string): Promise<string> => {
...
let notionFIle = 'https://s3-us-west-2.amazonaws.com/secure.notion-static.com/dc78e763-c029-43e7-823c-59d0af9b72be/1.jpg.jpg'

tempUrl(notionFIle,'3333.jpg',currentCookieString)
